# How Sentence Focus reads a page

The detail behind the short account in the [README](../README.md): what the
plugin does with a page of a PDF, a book or a web page, and why each rule is
there. Every rule below was put in by a document that broke the one before it.
...
- In books and web pages the highlight is the browser's own, which takes colour
  and decoration but no shape, so the rounded, soft and marker styles come out
  as the flat tint there.
